...
export const AddItemForm: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const processBarcodeImage = async (imageSrc: string) => {
        ...
        try {
            ...
            alert(`Failed to process barcode: ${errMsg}`);
        } finally {
    ...
    const processExpiryImage = async (imageSrc: string) => {
        ...
        try {
            ...
            const errMsg = err instanceof Error ? err.message : 'Unknown error';
            alert(`Failed to process image: ${errMsg}`);
        } finally {
            setScanningType(null);
            setScanStatus('');
        }
    ...
    const handleCameraCapture = async (result: string) => {
        if (activeCameraType === 'product') {
            if (result.startsWith('data:image/') || result.length > 200) {
                processBarcodeImage(result);
            } else {
                // It is already the decoded barcode text!
                setScanningType('product');
                setScanStatus(`Barcode: ${result}. Looking up product...`);
                setDetectedBarcode(result);

                try {
                    const productInfo = await lookupProduct(result);
                    if (productInfo) {
                        setName(productInfo.name);
                        setCategory(productInfo.category);
                    ...
                    alert("Failed to process barcode lookup.");
                } finally {
                    setScanningType(null);
                    setScanStatus('');
                }
