...
export const LoginPage: React.FC = () => {
    ...
    useEffect(() => {
        const getClientIdAndInit = async () => {
            let clientId = GOOGLE_CLIENT_ID;
            if (!clientId) {
                try {
                    console.log('[Google Auth] VITE_GOOGLE_CLIENT_ID is empty, fetching from backend...');
                    const response = await fetch('/api/auth/google');
                    if (response.ok) {
                        const data = await response.json();
                        clientId = data.clientId || '';
                        console.log('[Google Auth] Fetched Google Client ID from backend successfully');
                    } else {
                        console.error('[Google Auth] Failed to fetch Client ID, status:', response.status);
                    }
                } catch (err) {
                    console.error('[Google Auth] Error fetching Client ID from backend:', err);
                }
            }

            if (!clientId) {
                console.warn('[Google Auth] Warning: Google Client ID is not configured. Google Sign-In button will not render.');
                return;
            }
        ...
        const loadGoogleScript = () => {
            ...
            script.async = true;
            script.defer = true;
            script.onload = getClientIdAndInit;
